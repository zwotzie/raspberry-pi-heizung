...
--
-- NOTE: the table/settings DDL below is kept in sync with the DDL constant in
--       src/heizung/db.py (init_db runs it on API startup). Keep both aligned.
...
-- NOTE: UNIQUE (measured_at) already creates a btree index that Postgres can
-- scan backwards for ORDER BY ... DESC, so this extra DESC index is largely
-- redundant. It is kept to stay in sync with the DDL in src/heizung/db.py.
CREATE INDEX IF NOT EXISTS idx_measurements_measured_at
    ON measurements (measured_at DESC);

-- ── convenience helpers for manual psql queries ──────────────────────────────
-- These are NOT used by the application (the API issues its own SQL); they just
-- make ad-hoc inspection easier and return full measurement rows.
-- Usage:
